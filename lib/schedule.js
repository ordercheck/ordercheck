...
formSchedule.scheduleJob(resetDate, async function () {
  ...
  findWillFreePlan.forEach(async (data) => {
    ...
    if (!data.free_plan) {
      ...
      await db.plan.destroy({ where: { idx: data.idx } });
      db.plan.create({
      ...
      await db.plan.destroy({ where: { company_idx: data.company_idx } });
...
formSchedule.scheduleJob(tenAlarmDate, async function () {
  ...
  findAllCompany.forEach(async (data) => {
    // 알람 설정 체크
    const checkAlarm = await db.user.findByPk(data.user.idx);
    ...
    if (checkAlarm.kakaoCustomerStatusAlarm) {
      userReportKakaoPush(
    ...
    if (checkAlarm.emailCustomerStatusAlarm) {
      sendReport(
        data.compnay_name,
        "버튼 링크",
        newConsulting,
