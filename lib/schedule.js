...
const twoAlarmDate = "0 0 17 * * *";

const tenAlarmDate = "0 0 1 * * *";
const testAlarm = "*/1 * * * * *";

console.log("2시 매달 초기화 스케줄러");
formSchedule.scheduleJob(twoAlarmDate, async function () {});

console.log("1시 알람 스케줄러");
formSchedule.scheduleJob(oneAlarmDate, async function () {
  ...
  findFreePlan.forEach(async (data) => {
    ...
    if (diffTime == 30 || diffTime == 7 || diffTime == 1) {
      ...
      alarm.createAlarm({
        ...
        path: `${findCompanyResult.company_subdomain}/setting/manage_subscribe`,
      });
    }
  });
  // 2일동안 공유되지 않은 견
  ...
  findCalculate.forEach(async (data) => {
    const checkCompany = await db.company.findByPk(data.company_idx, {
    ...
    const insertData = {
      message,
      alarm_type: 12,
      path: `${checkCompany.company_subdomain}/custom_manage/detail/${data.customer.idx}/estimate`,
