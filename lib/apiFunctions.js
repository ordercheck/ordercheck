...
const { Company } = require("./classes/CompanyClass");

module.exports = {
  // 필터링 유저 체크
  checkUserCompany: async (company_idx, user_idx) => {
    const checkResult = await db.userCompany.findOne({
      where: { company_idx, user_idx, active: true, standBy: false },
  ...
  findWhiteFormDetail: async (formId, company_idx) => {
    let formDetail = await db.formLink.findOne({
      where: { idx: formId },
      include: [
        {
          model: db.formOpen,
    ...
    formDetail.whiteLabelChecked = findCompany.whiteLabelChecked;
    formDetail.member.forEach((data) => {
  ...
  createRandomCompany: async (huidx) => {
    const resetDate = moment().add("1", "M");
  ...
  includeUserToCompany: async (data) => {
    return await db.userCompany.create(data);
  },

  createFreePlan: async (company_idx) => {
    const merchant_uid = generateRandomCode();
    const start_plan = moment().format("YYYY.MM.DD");
  ...
  addUserId: (customerData, addminus, No) => {
    ...
    customerData = customerData.map((data) => {
      ...
      data.No = No;
      addminus == "plus" ? No++ : No--;
      data.customer_phoneNumber = data.customer_phoneNumber;
  ...
  setPlanDate: async (payType) => {
    ...
    return { nowStartPlan, nowExpirePlan };
  },
};
