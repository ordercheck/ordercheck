...
module.exports = {
  ...
  sendAlarm: async (message, companyIdx, alarm_type, io) => {
    try {
      const createAlarm = await db.alarm.create({
        message,
      ...
      const Alarm = {};
      Alarm.message = createAlarm.message;
      Alarm.idx = createAlarm.idx;
      Alarm.alarm_type = createAlarm.alarm_type;
      Alarm.createdAt = createAlarm.createdAt;
      console.log(Alarm);
      io.to(companyIdx).emit('sendAlarm', Alarm);
    } catch (err) {
      console.log(err);
    }
  },
