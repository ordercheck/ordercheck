...
module.exports = {
  ...
  findMembers: async (whereData, sortData, user) => {
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      result[i].No = No + 1;
      No = No + 1;
      result[i].template_name = result[i].config.template_name;
      result[i].templateId = result[i].config.idx;
      result[i].user_name = result[i].user.user_name;
      result[i].user_phone = result[i].user.user_phone;
      result[i].user_email = result[i].user.user_email;
      result[i].personal_code = result[i].user.personal_code;
      result[i].user_profile = result[i].user.user_profile;
      delete result[i].user;
      delete result[i].config;
  ...
  sendCompanyAlarm: async (insertData, findMembers, io) => {
    try {
      findMembers.forEach(async (data) => {
        ...
        io.to(data.user_idx).emit('addAlarm', alarm);
      });
    } catch (err) {
      console.log(err);
    }
