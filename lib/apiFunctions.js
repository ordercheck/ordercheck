  ...
  sendCompanyAlarm: async (
    ...
    try {
      findMembers.forEach(async (data) => {
        ...
        data.alarmId = createResult.idx;
        data.repeat_time = createResult.repeat_time;
        data.message = createResult.message;
        data.createdAt = createResult.createdAt;
        data.alarm_type = createResult.alarm_type;
        data.confirm = createResult.confirm;
        io.to(data.user_idx).emit('sendAlarm', data);
      });
    } catch (err) {
      console.log(err);
    }
