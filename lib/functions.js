...
const ks = process.env.KS;
const ka = process.env.KA;
const si = process.env.SI;
...
module.exports = {
  ...
  kakaoPush: async (up, templateCode, message, buttonExist, buttonsData) => {
    let user_phone = up;

    let data = {
      plusFriendId: "@오더체크",
      templateCode,
      messages: buttonExist
        ? [
            {
              countryCode: "+82",
    ...
    console.log(data.messages[0].buttons);
    ...
    try {
      ...
      return { success: true, data: sendResult.data.messages[0].messageId };
    } catch (err) {
      return { success: false, err };
    }
  },
  kakaoGetResult: async (messageId) => {
    const date = Date.now().toString();
    var space = " "; // one space
    var newLine = "\n"; // new line
    var method = "GET"; // method
    var url = `/alimtalk/v2/services/ncp:kkobizmsg:kr:2763215:ordercheck/messages/${messageId.data}`; // url (include query string)
    var timestamp = date; // current timestamp (epoch)
    var accessKey = ka; // access key id (from portal or Sub Account)
    var secretKey = ks; // secret key (from portal or Sub Account)
    ...
    try {
      const result = await axios({
        url: `https://sens.apigw.ntruss.com/alimtalk/v2/services/ncp:kkobizmsg:kr:2763215:ordercheck/messages/${messageId.data}`,
      ...
      return { succes: true, sendResult: result.data.messageStatusCode };
    } catch (err) {
      return { succes: false, err };
    }
  },
  smsPush: async (user_phone, message, type) => {
