  ...
  multer_calculate_upload: () => {
    const calculateUpload = multer({
      storage: multerS3({
        ...
        key(req, file, cb) {
          cb(null, `calculate/${req.params.customer_idx}/${_f.random5()}`);
        },
      }),
    });
  ...
  multer_company_Enrollment_upload: () => {
    const calculateUpload = multer({
      storage: multerS3({
        ...
        contentType: multerS3.AUTO_CONTENT_TYPE,
        key(req, file, cb) {
          cb(null, `enrollment/${req.company_idx}/${file.originalname}`);
        },
      }),
    });
    return calculateUpload;
    ...
    const thumbNailUpload = multer({
      storage: multerS3({
        ...
        key(req, file, cb) {
          ...
          cb(null, `original/${_f.random5()}${file.originalname}`);
        },
      }),
    });
  ...
  multer_upload_formLink_img: () => {
    const thumbNailUpload = multer({
      storage: multerS3({
        ...
        contentType: multerS3.AUTO_CONTENT_TYPE,
        key(req, file, cb) {
          cb(null, `formLink/${_f.random5()}${file.originalname}`);
        },
      }),
    });
    return thumbNailUpload;
  ...
  multer_file_store_upload: () => {
    const fileStoreUpload = multer({
      storage: multerS3({
        ...
        key(req, file, cb) {
          if (!req.query.path) {
            cb(null, `fileStore/${_f.random5()}${file.originalname}`);
          }
        },
      }),
    });
    return fileStoreUpload;
  },
