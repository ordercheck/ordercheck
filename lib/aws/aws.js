  ...
  multer_calculate_upload: () => {
    const calculateUpload = multer({
      storage: multerS3({
        ...
        key(req, file, cb) {
          cb(null, `calculate/${_f.random5()}${file.originalname}`);
        },
      }),
    });
        ...
        transforms: [
          {
            ...
            key: function (req, file, cb) {
              cb(null, `logo/${_f.random5()}${file.originalname}`); //use Date.now() for unique file keys
            },
      ...
      storage: multerS3({
        ...
        key(req, file, cb) {
          if (!req.query.path) {
            cb(
              null,
              `fileStore/${req.params.customerFile_idx}/${file.originalname}`
            );
          } else {
            cb(
              null,
              `fileStore/${req.params.customerFile_idx}/${req.query.path}/${file.originalname}`
            );
          }
        },
      }),
