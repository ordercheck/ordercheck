...
const run = (sec) => {
  schedule.scheduleJob(`*/${sec} * * * * *`, async () => {
      try {
        ...
        let push = await db.sequelize.query(`select * from pushNotifies where status = 0 order by idx limit 1`).spread((r)=>{
            return _f.makeSpreadArray(r)
        })
        if(push.length > 0){
            let p = push[0];
            if(p.category == 3){
                let user = await db.users.findAll({where:{idx:p.uidx}}).then((r)=>{
                    return makeArray(r)[0]
                })
    
                if(user.pushActive == 0){
                    await db.pushNotifies.update({status:1},{where:{idx:push[0].idx}})
                }else{
                    await axios({
