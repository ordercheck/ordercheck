...
const run = (sec) => {
  schedule.scheduleJob(`*/${sec} * * * * *`, async () => {
    ...
    let kakaoPush = await db.sequelize.query(`select * from kakaoPushes where active = 0 limit 1`).spread((r)=>{
    ...
    if(kakaoPush.length > 0){
        ...
        if(template_code == "pushb02"){
            ...
            var url = "/alimtalk/v2/services/ncp:kkobizmsg:kr:2763215:ordercheck/messages";	// url (include query string)
            ...
            await db.kakaoPush.update({active:1},{where:{idx:kakaoPush[0].idx}})
        } catch (error) {
            console.log(error)
            await db.kakaoPush.update({active:1,option:JSON.stringify(error)},{where:{idx:kakaoPush[0].idx}})
        }
