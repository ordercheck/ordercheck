...
const run = (sec) => {
  schedule.scheduleJob(`*/${sec} * * * * *`, async () => {
    ...
    let image = await db.sequelize.query(`select * from images where upload = 0 and error = 0 limit 1`).spread((r)=>{
    ...
    if(image.length > 0){
        ...
        await FileUpload.ufile.blobUpload(query, async (err, url) => {
          if(err){
            ...
            console.log(url)
            await db.images.update({upload:1,original_image:url.original,original_image_bi:null},{where:{image_idx}})
          }
        });
