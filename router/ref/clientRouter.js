...
router.get("/", async (req, res, next) => {
  ...
  let data = await db.sequelize
    .query(
      `SELECT * FROM totalcases where active = 1 order by idx desc limit 4`
    )
    .spread((r) => {
      return makeSpreadArray(r);
    });
  let banner = await db.sequelize
    .query(
      `SELECT * FROM banners where active = 1 and type = 0 order by banners.order`
    )
    .spread((r) => {
      return makeSpreadArray(r);
    });
  ...
  let cidx = 1;
  let company_data = await db.companies
    .findAll({ where: { idx: cidx } })
    .then((r) => {
  ...
  res.render("client/type01/normal/index", {
    data,
    flag,
    banner,
    company_data,
...
router.get("/eventlist", async (req, res, next) => {
  ...
  let data;
  if (active == 1) {
    data = await db.sequelize
      .query(
        `SELECT * FROM byjieun_home.events where date(end) > now() and active = 1`
      )
      .spread((r) => {
        return makeSpreadArray(r);
      });
    ...
    data = await db.sequelize
      .query(
        `SELECT * FROM byjieun_home.events where date(end) < now() or active = 0`
      )
      .spread((r) => {
        return makeSpreadArray(r);
      });
...
router.get("/totalcase", async (req, res, next) => {
  ...
  if (category == "전체") {
    ...
    data = await db.sequelize
      .query(
        `select * from totalcases where active = 1 and category like '%${category}%'`
      )
      .spread((r) => {
        return makeSpreadArray(r);
      });
  }
  res.render("client/type01/pc/totalcase", {
    company_data,
    data,
    category,
    flag,
