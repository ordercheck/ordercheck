...
const { makeArray, makeSpreadArray, randomString9 } = require("../../lib/functions");
...
router.post("/login", async (req,res)=>{
    ...
    let result = await db.companies.findAll({where:{company_id,company_password}}).then((r)=>{
        return makeArray(r)
    })
    if(result.length > 0){
...
router.post("/base/main", check_data, async (req,res)=>{
    const {title,subTitle,logo,kakao_thumb,naver,instagram,facebook,footer} = req.body;
    const token = req.query.token;
    const company_data = await verify_data(token)
    const cidx = company_data.idx;
    await db.companies.update({title,subTitle,logo,kakao_thumb,naver,instagram,facebook,footer},{where:{idx:cidx}})
    res.send({success:200})
})
...
router.post("/totalcase/upload", check_data, async (req,res)=>{
    ...
    let before = req.body._before_array == undefined ? '' : req.body._before_array.join("**by**")
    ...
    if(code == ''){
        code = randomString9();
        await db.totalcases.create({cidx,title,highlight,category,price,pyeung,date,term,term_type,thumbnail,before,after,code})
    }else{
        await db.totalcases.update({title,highlight,category,price,pyeung,date,term,term_type,thumbnail,before,after},{where:{code,cidx}})
    }
    ...
    try {
        ...
        if(code == ''){
            ...
            await db.events.create({
                cidx,title,thumbnail,image,start,end,code,link
            })
            res.send({success:200,code})
        }else{
            await db.events.update({title,thumbnail,image,start,end,link},{where:{code}})
            res.send({success:200,code})
        }
    ...
    try {
        ...
        if(code == ''){
            ...
            await db.banners.create({
                cidx,title,link,banner,mobile_banner,code,
            })
            res.send({success:200,code})
        }else{
            await db.banners.update({title,link,banner,mobile_banner},{where:{code}})
            res.send({success:200,code})
        }
